...
import { FaPlus, FaSearch, FaPen, FaTrash, FaChevronLeft, FaChevronRight, FaLayerGroup, FaChevronDown, FaUserTie, FaUserGraduate } from 'react-icons/fa';
...
export const SubjectList = () => {
  ...
  const handleStudentSelection = (studentId) => {
    ...
    console.log('Selected students after change:', [...selectedStudents, studentId]);
  };
  ...
  const renderStudentView = () => (
    <div className="subject-management">
      <div className="subject-management__header-row">
        <h2 className="subject-management__title">Manage Subjects</h2>
        <button 
          className="subject-management__back-btn"
          onClick={() => setViewMode('selection')}
        >
          Go Back Selection Area
        </button>
      </div>
      
      <div className="subject-management__actions-row">
  ...
  const renderTeacherView = () => (
    <div className="subject-management">
      ...
      <div className="subject-management__controls-row">
        <div className="subject-management__search">
          <input 
            type="text" 
            placeholder="Search subjects..."
  ...
  const handleAddSubject = async (e) => {
    ...
    try {
      ...
      if (viewMode === 'student' && selectedStudents.length === 0) {
      ...
      // Log the students being added
      console.log('Adding subject with students:', selectedStudents);
      
      // Create a new subject with ID for local state
      const newSubjectWithId = {
        ...subjectToAdd,
        id: `subj${Date.now()}` // Use timestamp for unique ID
      };
      ...
      try {
        ...
        // Update the local subject with the Firebase ID
        if (subjectId) {
          setSubjects(prevSubjects => 
            prevSubjects.map(s => 
              s.id === newSubjectWithId.id ? {...s, id: subjectId} : s
            )
          );
        }
      } catch (firebaseErr) {
        console.error('Firebase error, but subject added to local state:', firebaseErr);
        // We already added to local state, so just log the error
      }
    ...
    try {
      ...
      setSubjects(prevSubjects => 
        prevSubjects.map(subject => 
          subject.id === currentSubject.id ? updatedSubjectData : subject
        )
      );
      
      // Try to update in Firebase
      try {
        await updateSubject(currentSubject.id, updatedSubjectData);
        console.log('Subject updated successfully in Firebase');
      } catch (firebaseErr) {
        console.error('Firebase error updating subject:', firebaseErr);
        // We already updated the local state, so just log the error
      }
        ...
        <div className="subject-mgmt-modal">
          <div className="subject-mgmt-modal__content">
            ...
            <form onSubmit={saveEditedSubject}>
              ...
              <div className="subject-mgmt-modal__form-group">
                <label>{viewMode === 'teacher' ? 'Assigned Teacher' : 'Assigned Students'}</label>
                {viewMode === 'teacher' ? (
                  <select
